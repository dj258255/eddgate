...
| 메뉴 | 설명 |
|------|------|
| **실행** | 워크플로우, 모델, 노력 수준, 사고 모드, 입력 파일/텍스트 선택. 라이브 대시보드에서 단계별 진행(왼쪽) + 스트리밍 로그(오른쪽). 헤더에서 경과 시간, 토큰, 비용 추적. |
| **분석** | 컨텍스트 프로파일러 또는 실패 분석. 결과를 분할 뷰로 표시. |
| **테스트** | 현재 동작 스냅샷, 기준선 비교, 저장된 스냅샷 목록. |
| **MCP** | YAML 편집 없이 MCP 서버 추가/제거/목록. |
| **플러그인** | 설치된 워크플로우, 역할, Claude Code 플러그인 확인. 파일 브라우저로 가져오기. |
| **설정** | 기본 모델, 언어(한국어/영어), 트레이스 설정 변경. |
