...
| **분석** | 실패 분석, 컨텍스트 프로파일러, 오프라인 평가(LLM 판사로 트레이스 재평가), 커밋 간 평가 비교(diff-eval), 프롬프트/워크플로우 변경 비교(version-diff). |
...
| **플러그인** | 워크플로우/역할 확인, 워크플로우 시각화(ASCII 다이어그램), 단일 단계 디버그, 파일에서 가져오기. |
